...
public class TokenAuthenticationUserDetailsService
	...
	public UserDetails loadUserDetails(PreAuthenticatedAuthenticationToken authentication)
		...
		log.info("loadUserDetails {TokenAuthenticationUserDetailsService: 33}");
		if (authentication.getPrincipal() != null && authentication.getPrincipal() instanceof String
			...
			return new TokenUserDetails(token.getSubject(), token.getClaim("usr").asString(),
					(String) authentication.getCredentials(), token.getToken(), true,
					token.getClaim("role").asList(String.class).stream().map(SimpleGrantedAuthority::new)
