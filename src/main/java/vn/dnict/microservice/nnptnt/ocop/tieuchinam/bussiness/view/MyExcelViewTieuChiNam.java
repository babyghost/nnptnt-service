package vn.dnict.microservice.nnptnt.ocop.tieuchinam.bussiness.view;
...
public class MyExcelViewTieuChiNam extends AbstractXlsView{
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		cell.setCellStyle(styleTitleB14);
		cell.setCellValue("BÁO CÁO DANH SÁCH TIÊU CHÍ ĐÁNH GIÁ " + formattedString);
		sheet.addMergedRegion(new CellRangeAddress(currentRow, // first row (0-based)
				currentRow, // last row (0-based)
				0, // first column (0-based)
		...
		currentColumn++;
		List<TieuChiNamData> tieuChiNamDatas = (List<TieuChiNamData>) model
				.get("tieuChiNamDatas");
		int stt = 1;
		if (Objects.nonNull(tieuChiNamDatas) && !tieuChiNamDatas.isEmpty()) {

			for (TieuChiNamData tieuChiNamData : tieuChiNamDatas) {
				currentRow++;
				currentColumn = 0;
				...
				cell2.setCellValue(
						tieuChiNamData.getNam() );
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(
						tieuChiNamData.getNganhHangTen()  != null ? tieuChiNamData.getNganhHangTen() : "");
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(
						tieuChiNamData.getNhomTen() != null ? tieuChiNamData.getNhomTen() : "");
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(
						tieuChiNamData.getPhanNhomTen() != null ? tieuChiNamData.getPhanNhomTen()  : "");
				currentColumn++;

				
				Integer trangThai = tieuChiNamData.getTrangThai();
				String trangThais;
				if(trangThai== 0) {
