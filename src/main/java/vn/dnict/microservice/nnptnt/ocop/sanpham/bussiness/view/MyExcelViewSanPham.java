package vn.dnict.microservice.nnptnt.ocop.sanpham.bussiness.view;
...
public class MyExcelViewSanPham extends AbstractXlsView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		cell.setCellStyle(styleTitleB14);
		cell.setCellValue("BÁO CÁO THÔNG TIN SẢN PHẨM NGÀY " + formattedString);
		sheet.addMergedRegion(new CellRangeAddress(currentRow, // first row (0-based)
				currentRow, // last row (0-based)
				0, // first column (0-based)
		...
		cell2 = headerRow.createCell(currentColumn);
		cell2.setCellStyle(styleHeaderB);
		cell2.setCellValue("Chủ Thể");
		currentColumn++;

		cell2 = headerRow.createCell(currentColumn);
		cell2.setCellStyle(styleHeaderB);
		cell2.setCellValue("Ngành Hàng ");
		currentColumn++;
		...
		cell2 = headerRow.createCell(currentColumn);
		cell2.setCellStyle(styleHeaderB);
		cell2.setCellValue("Phân Nhóm ");
		currentColumn++;

		cell2 = headerRow.createCell(currentColumn);
		cell2.setCellStyle(styleHeaderB);
		cell2.setCellValue("Phân Hạng ");
		currentColumn++;

		cell2 = headerRow.createCell(currentColumn);
		cell2.setCellStyle(styleHeaderB);
		cell2.setCellValue("Số Quyết Định ");
		currentColumn++;

		cell2 = headerRow.createCell(currentColumn);
		cell2.setCellStyle(styleHeaderB);
		cell2.setCellValue("Trạng Thái ");
		currentColumn++;
		List<SanPhamData> sanPhamDatas = (List<SanPhamData>) model.get("SanPhamDatas");
		int stt = 1;
		if (Objects.nonNull(sanPhamDatas) && !sanPhamDatas.isEmpty()) {

			for (SanPhamData sanPhamData : sanPhamDatas) {
				currentRow++;
				currentColumn = 0;
				...
				cell2.setCellValue(sanPhamData.getTen() != null ? sanPhamData.getTen() : "");
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(sanPhamData.getDoanhNghiepTen() != null ? sanPhamData.getDoanhNghiepTen() : "");
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(sanPhamData.getNganhHangTen() != null ? sanPhamData.getNganhHangTen() : "");
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(
						sanPhamData.getNhomTen() != null ? sanPhamData.getNhomTen() : "");
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(sanPhamData.getPhanNhomTen() != null ? sanPhamData.getPhanNhomTen() : "");
				currentColumn++;
				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue( sanPhamData.getPhanHangTen() != null ? sanPhamData.getPhanHangTen() : "");
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(sanPhamData.getQuyetDinh() != null ? sanPhamData.getQuyetDinh() : "");
				currentColumn++;
				Integer trangThai = sanPhamData.getTrangThai();
//				String trangThais;
//				if (trangThai == 0) {
//					
...
//				}
				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(trangThai != null && trangThai == 1 ? " Không Hoạt động" : " Đang hoạt động");
				currentColumn++;
