package vn.dnict.microservice.nnptnt.kehoach.nhiemvuthang.business.view;
...
public class MyExcelViewThongKeThang extends AbstractXlsView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		cell.setCellStyle(styleTitleB14);
		cell.setCellValue("BÁO CÁO TÌNH HÌNH THỰC HIỆN KẾ HOẠCH RIÊNG CỦA PHÒNG ");
		sheet.addMergedRegion(new CellRangeAddress(currentRow, // first row (0-based)
				currentRow, // last row (0-based)
				0, // first column (0-based)
				5 // last column (0-based)
		));
		List<ThongKeKeHoachThangData> thongKeThangDatas = (List<ThongKeKeHoachThangData>) model
				.get("thongKeThangDatas");
		...
		cell2 = headerRow.createCell(currentColumn);
		cell2.setCellStyle(styleHeaderB);
		cell2.setCellValue("Phân công thực hiện");
		currentColumn++;
		...
		cell2 = headerRow.createCell(currentColumn);
		cell2.setCellStyle(styleHeaderB);
		cell2.setCellValue("Thời hạn hoàn thành");
		currentColumn++;
		...
		int stt = 1;
		if (Objects.nonNull(thongKeThangDatas) && !thongKeThangDatas.isEmpty()) {

			for (ThongKeKeHoachThangData thongKeThangData : thongKeThangDatas) {
				currentRow++;
				currentColumn = 0;
				...
				cell2.setCellValue(
						thongKeThangData.getTenNhiemVu() != null ? thongKeThangData.getTenNhiemVu() : "");
				currentColumn++;
				
				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(
						thongKeThangData.getCanBoThucHienTen() != null ? thongKeThangData.getCanBoThucHienTen() : "");
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(
						thongKeThangData.getDonViPhoiHop() != null ? thongKeThangData.getDonViPhoiHop() : "");
				currentColumn++;

				String thoihan = "";
				if(Objects.nonNull(thongKeThangData.getThoiGian())) {
					thoihan = formatter.format(thongKeThangData.getThoiGian());
				}
				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(thoihan);
				currentColumn++;

				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(thongKeThangData.getGhiChu());
				currentColumn++;
				String tinhTrangTen="";
				if(thongKeThangData.getTinhTrang() !=null) {
					if(thongKeThangData.getTinhTrang() == 1) {						
						tinhTrangTen="Đang thực hiện";
					}else if(thongKeThangData.getTinhTrang() == 2) {
						tinhTrangTen="Đã hoàn thành";
					}else if(thongKeThangData.getTinhTrang() == 3) {
						tinhTrangTen="Ngừng thực hiện";
					} else {
				...
				if(thongKeThangData.getTienDoMucDoHoanThanh() != null) {
					mucDoHoanThanh = thongKeThangData.getTienDoMucDoHoanThanh();
				...
				cell2 = headerRow.createCell(currentColumn);
				cell2.setCellStyle(styleCellCenter);
				cell2.setCellValue(thongKeThangData.getTienDoKetQua());
				currentColumn++;
