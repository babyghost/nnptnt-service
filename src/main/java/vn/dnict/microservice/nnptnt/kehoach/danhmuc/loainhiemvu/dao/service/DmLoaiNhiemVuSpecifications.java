package vn.dnict.microservice.nnptnt.kehoach.danhmuc.loainhiemvu.dao.service;
...
public class DmLoaiNhiemVuSpecifications {
	public static Specification<DmLoaiNhiemVu> quichSearch(final String search, final Boolean trangThai, final Boolean isDefault) {
		return new Specification<DmLoaiNhiemVu>() {
			...
			public Predicate toPredicate(Root<DmLoaiNhiemVu> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (isDefault != null) {
					predicates.add(cb.equal(root.<String>get("isDefault"), isDefault));
				}
