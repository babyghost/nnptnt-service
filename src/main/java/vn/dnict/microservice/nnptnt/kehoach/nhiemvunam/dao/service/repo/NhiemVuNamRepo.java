...
public interface NhiemVuNamRepo extends JpaRepository<NhiemVuNam, Long>, JpaSpecificationExecutor<NhiemVuNam> {
	...
	@Query("SELECT u FROM NhiemVuNam u WHERE u.keHoachId = ?1 AND u.daXoa = ?2")
	public List<NhiemVuNam> findByKeHoachIdAndDaXoa(Long keHoachId, Boolean daXoa);

	@Modifying(clearAutomatically = true)
	@Query("update NhiemVuNam u set u.daXoa = ?1 where u.keHoachId = ?2")
	public int setFixedDaXoaForKeHoachId(Boolean daXoa, Long keHoachId);

	@Query("SELECT u FROM NhiemVuNam u WHERE u.keHoachId = ?1 AND u.nhiemVuChaId IS NULL AND u.daXoa = ?2 ORDER BY u.sapXep ASC")
	public List<NhiemVuNam> findByKeHoachIdAndNhiemVuChaIdIsNullAndDaXoa(Long keHoachId, Boolean daXoa);

	@Query("SELECT u FROM NhiemVuNam u WHERE u.keHoachId = ?1 AND u.nhiemVuChaId = ?2 AND u.daXoa = ?3 ORDER BY u.sapXep ASC")
	public List<NhiemVuNam> findByKeHoachIdAndNhiemVuChaIdAndDaXoa(Long keHoachId, Long nhiemVuChaId, Boolean daXoa);

	@Query("SELECT u FROM NhiemVuNam u WHERE u.keHoachId = ?1 AND u.loaiNhiemVuId = ?2 AND  u.nhiemVuChaId IS NULL AND u.daXoa = ?3 ORDER BY u.sapXep ASC")
	public List<NhiemVuNam> findByKeHoachIdAndLoaiNhiemVuIdAndNhiemVuChaIdIsNullAndDaXoa(Long keHoachId, Long loaiNhiemVuId,
			Boolean daXoa);

	@Query("SELECT u FROM NhiemVuNam u WHERE u.keHoachId = ?1 AND u.loaiNhiemVuId = ?2 AND u.nhiemVuChaId = ?3 AND u.daXoa = ?4 ORDER BY u.sapXep ASC")
